...
if ($USER->IsAdmin() && !_::isEmpty($tasks)) {
    $iblockFields = function($iblockType, $code, $name) {
        return array (
            'ACTIVE' => 'Y',
            'NAME' => $name,
            'CODE' => $code,
            'LIST_PAGE_URL' => '#SITE_DIR#/hotel/',
            'DETAIL_PAGE_URL' => '#SITE_DIR#/hotel/',
            'CANONICAL_PAGE_URL' => '',
            'INDEX_ELEMENT' => 'Y',
            ...
            'SECTION_PAGE_URL' => '#SITE_DIR#/hotel/',
            'INDEX_SECTION' => 'Y',
    ...
    $commentProp = ['code' => 'COMMENT', 'name' => 'Примечание'];
    $iblocks = [
        [
            'code' => 'additional',
            'name' => 'Дополнительные услуги',
            'props' => [$priceProp, $commentProp]
        ],
    ];
    $sections = [
        [
    ...
    if (in_array('iblocks', $tasks)) {
        ...
        try {
            foreach ($iblocks as $iblock) {
                $ib = new CIBlock();
                $iblockId = $ib->Add($iblockFields(\App\Iblock::HOTEL_SERVICES, $iblock['code'], $iblock['name']));
